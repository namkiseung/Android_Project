package com.namkit.namki.teamnova.Login;
...
import com.namkit.namki.teamnova.FindMember.FindMember;
import com.namkit.namki.teamnova.Menu1_MainActivity.MainActivity;
import com.namkit.namki.teamnova.Menu3_Daily.Daily_MainItem;
import com.namkit.namki.teamnova.R;
import com.namkit.namki.teamnova.Register.Signup;
...
public class Login extends Activity implements View.OnClickListener {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (buttonView.getId() == R.id.Auto_MemoId) {
                    if (isChecked) {
                        ...
                        System.out.println("(if)editor.putString(\"basiccheck\", \"true\");의 값은 : " + preferences.getString("basiccheck", "" + "    11111111111111111111"));
                        System.out.println("(if)editor.putString(\"basicid\",etId.getText().toString())의 값은 : " + preferences.getString("basicid", "" + "    11111111111111111111"));
                        ...
                        System.out.println("(else)editor.putString(\"basiccheck\", \"false\");의 값은 : " + preferences.getString("basiccheck", "" + "    11111111111111111111"));
                        System.out.println("(else)editor.putString(\"basicid\",\"\");의 값은 : " + preferences.getString("basicid", "" + "    11111111111111111111"));
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn_login:
                ...
                System.out.println("(onclick)preferences.getString(tempID + \"TEAM_MEMBER_ID\", \"\");의 값은 : " + preferences.getString(tempID + "TEAM_MEMBER_ID", "") + "    11111111111111111111");
                System.out.println("(onclick)preferences.getString(tempID + \"TEAM_MEMBER_PW\", \"\")의 값은 : " + preferences.getString(tempID + "TEAM_MEMBER_PW", "") + "    11111111111111111111");
                if (!tempID.equals("") || !tempPW.equals("")) {//아이디와 비번 공백이 넘어올때의 처리
                    if (tempID.equals(displayId) && tempPW.equals(displaypwd) || tempID.equals("user")) {//아이디와 비번이 맞는지
                        if (tempID.equals(displayId) || tempID.equals("user")) { //가입이 되어있는지 확인
                            ...
                            System.out.println("(1)(onclick깊숙)editor.putString(\"currentdisplayId\", displayId)의 값은 : " + preferences.getString("currentdisplayId", displayId) + "    11111111111111111111");
                            editor.apply();
                            System.out.println("(onclick깊숙)editor.putString(\"currentdisplayId\", displayId)의 값은 : " + preferences.getString("currentdisplayId", displayId) + "    11111111111111111111");
                            CountTask count = new CountTask();  //AsyncTask상속받은 클래스를 사용하자
                            count.execute();//count 객체 실행
                            etId.setText("");
                            etPwd.setText("");
                            startActivityForResult(login, 69);
                        } else {//가입이 안되었을때
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent dataintent) {
        ...
        if (requestCode == 69) {
            if (resultCode == RESULT_OK) {
                ...
                if (preferences.getString("basiccheck","").equals("true")) {
                    String memoId = dataintent.getStringExtra("logoutId");
                    etId.setText(memoId);
                    //editor.putString("currentdisplayId", displayId);
                }
   ...
   /* @Override
    ...
    private void checkSharedPreferences() {
        ...
        System.out.println("(로그인시 저장된값)preferences.getString(\"basiccheck\", \"false\")의 값은 : " + preferences.getString("basiccheck", "false") + "    11111111111111111111");
        System.out.println("(로그인시 저장된값)preferences.getString(\"basicid\", \"\")의 값은 : " + preferences.getString("basicid", "") + "    11111111111111111111");
